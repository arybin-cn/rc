...
# 1. Select Protocol with Validation
while true; do
    echo "--------------------------------------------------"
    echo "Select the protocol you want to deploy:"
    echo "1) REALITY (VLESS + XTLS Vision)"
...
if [ "$PROTO_CHOICE" == "1" ]; then
    ...
    read -p "Enter destination [default: dash.cloudflare.com:2053]: " DESTINATION
    ...
    [ $? -ne 0 ] && { echo "Error: Key generation failed!"; exit 1; }
